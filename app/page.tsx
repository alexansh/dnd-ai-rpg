...
import { Flame, Sparkles, RotateCcw, Shield, Swords } from "lucide-react";

export default function GamePage() {
  const { openCreation, resetGame, isCombatActive, combat } = useGameStore();
      ...
      <header className="h-12 bg-obsidian-950 border-b border-gold-500/30 px-6 flex items-center justify-between z-40 shadow-md">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-gold-400">
            ...
            <span className="font-cinzel text-base font-extrabold tracking-wider bg-gradient-to-r from-gold-400 via-gold-300 to-amber-500 bg-clip-text text-transparent">
              THE WAYWARD FLAGON
            </span>
          </div>
          <span className="text-[10px] uppercase tracking-widest font-mono text-parchment-300/40 px-2 py-0.5 rounded bg-obsidian-900 border border-gold-500/20">
            5e Tactical AI RPG
          </span>
        </div>
